/******************************************************************************************************************************************
        ...
        Authors: Joshua Francis - FRAN0464
                 Ilan Grynszpan - GRYN0002
        ...
        -Code for queue structure, function now64(), and, other inspirations were taken from Tim Seeley's unreliable relay server and modified for use within this code.
        ...
        Altough it works fine on most of the tested cases, the following code might be missing a few features related to:
...
int BUFFERSIZE = 256, SEQ = 10000, HEADERSIZE = 11, TIMEOUT = 200, Qsize = 0, VERBOSE = 0, expected_SEQ = 10000;
long long timer;
...
void *ReceiveSTDIN(void *Vars) {
    ...
    char header[BUFFERSIZE + HEADERSIZE];
    unsigned short crc;
    ...
    while (1) {
        ...
        fgets(buffer, BUFFERSIZE - 1, stdin); // wait here

        /* Create header (5 Digit SEQ + 'M' or 'A' + 5 Digit CRC) */
        ...
        if (crc) {
            ...
            if (VERBOSE) {
                dump_queue(var->msq);
                fprintf(stderr, "Sending from STDIN %s \n", header);
            }
...
void *ReceiveSOCKET(void *Vars) {
    ...
    int n, i, j, k, l, m, crc_num, crc_rcvd, rcvd_SEQ_num;
    int ACK_SEQ = 0, ret = 0;
    char buffer[BUFFERSIZE], *msg;
    char crc[6], rcvd_SEQ[6], ACK[8];
    ...
    while (1) {
        ...
        if (ret > 0) {
            ...
            if (buffer[5] == 'A') {
                ...
                if (isDigit(buffer[0]) && isDigit(buffer[1]) && isDigit(buffer[2]) && isDigit(buffer[3]) && isDigit(buffer[4])) { // If ACK is not corrupt
                    struct mqn *m = var->msq->head;
                    ACK_SEQ = (buffer[0] - '0')*10000 + (buffer[1] - '0')*1000 + (buffer[2] - '0')*100 + (buffer[3] - '0')*10 + (buffer[4] - '0');
                ...
                for (i = 6; i <= 10; i++) crc[i - 6] = buffer[i];
                crc[5] = '\0';
                crc_num = atoi(crc);

                /* Store received MSG and calculate CRC */

                msg = (char*) malloc(sizeof (buffer) - 11);
                for (i = 11; i < sizeof (buffer); i++) msg[i - 11] = buffer[i];
                ...
                if (crc_num == crc_rcvd) { // If MSG is not corrupted              
                    if (rcvd_SEQ_num <= expected_SEQ) { // If SEQ is the one we're expecting (received in order) or an older msg (ACK didnt make it back)
                        ...
                        m = ((rcvd_SEQ_num % 100000) - l - k - j - i) / 10000;
                        ACK[0] = itoa(m);
                        ACK[1] = itoa(l);
                        ACK[2] = itoa(k);
                        ACK[3] = itoa(j);
...
void enqueue(struct mq *q, char *msg, int SEQ) {
    ...
    Qsize++;
    m->msg = (char*) malloc(HEADERSIZE + BUFFERSIZE);
    for (i = 0; i < strlen(msg); i++) {
        m->msg[i] = msg[i];
    }
    m->next = 0;
    m->SEQ = SEQ;
    if (VERBOSE) fprintf(stderr, "Queue size is now %d\n", Qsize);
